...
def prepare_column(
    ...
    if col in st.session_state.get("ordered_columns", set()):
        if analytical == "CONTINUOUS":
            analytical = "CONTINUOUS_ORDERED"
            report.analytical_type = analytical

    if analytical in ("TEMPORAL",) or (
        profile
        and profile.analytical_type == "TEMPORAL"
    ...
    if analytical == "CONTINUOUS" and series.dtype == object:
        ...
        rate = numeric.notna().sum() / max(len(series.dropna()), 1)
        if rate > 0.9:
            report.coerced_dtype = str(numeric.dtype)
            report.parse_rate = float(rate)
...
def detect_ohlc_columns(df: pd.DataFrame) -> dict | None:
    ...
    for key, patterns in [
        ...
        for c_lower, c_orig in cols_lower.items():
            if any(c_lower == p or c_lower.endswith(f"_{p}") for p in patterns):
                mapping[key] = c_orig
                break
    date_col = None
    for c in df.columns:
        ...
        if any(
            x in cl
            for x in ("date", "time", "timestamp", "datetime", "period")
        ):
            date_col = c
            break
...
def get_eligible_columns(
    ...
    for col, p in profiles.items():
        at = p.analytical_type
        if at in allowed_types or (include_none and "NONE" in allowed_types):
            result.append(col)
